...
void dijkstra(vec<ll> &dist, vec<vec<pair<int,ll>>> &g, int s) {
    priority_queue<pair<ll, int>, vec<pair<ll, int>>, greater<pair<ll, int>>> pq;
    dist[s] = 0LL; pq.push(make_pair(0LL,s));
    while(!pq.empty()) {
...
void solve(){
    ...
    vec<ll> distS(n+1, inf<ll>);
    vec<ll> distA(n+1, inf<ll>);
    vec<ll> distB(n+1, inf<ll>);

    dijkstra(distS, g, s);
    dijkstra(distA, g, a);
    dijkstra(distB, g, b);

    if((distS[a]==inf<ll>||distA[e]==inf<ll>)&&(distS[b]==inf<ll>||distB[e]==inf<ll>)) {
        cout<<-1<<endl;
        return;
    }
    cout<<min(distS[a]+distA[e], distS[b]+distB[e])<<endl;
}
