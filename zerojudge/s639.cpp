...
int backtracking(ll n, vec<ll> &v, ll now, ll w, int idx) {
    if(now==w) return 1;
    if(idx>=n||now>w) return 0;
    return backtracking(n, v, now+v[idx], w, idx+1) + backtracking(n, v, now, w, idx+1);
}
...
void solve() {
    ...
    vec<ll> v(n);
    for(auto &i: v) cin>>i;
    cout<<backtracking(n, v, 0LL, w, 0)<<endl;    
}
