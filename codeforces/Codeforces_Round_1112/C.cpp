...
bool chk(int idx, int place, int m, vec<pair<pii, pii>> &v) {
    if(v[idx].F.F<=place&&place<=v[idx].F.S) return false;
    if((v[idx].S.F<=m-place+1)&&(m-place+1<=v[idx].S.S)) return false;
    return true;
}

void solve() {
    int n; cin>>n;
    vec<pair<pii, pii>> v(n);
    for(auto &i: v) cin>>i.F.F>>i.F.S>>i.S.F>>i.S.S;
    for(int m=n;m>=1;m--) {
        int idx = 0; bool x = true;
        for(int i=1;i<=m;i++) {
            while(idx<n&&!chk(idx, i, m, v)) idx++;
            if(idx==n) {
                x = false;
                break;
            }
